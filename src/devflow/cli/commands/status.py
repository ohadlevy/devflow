...
def show_status(config: ProjectConfig, format: str = "table") -> Dict[str, Any]:
    ...
    project_info = {
        ...
        "maturity_level": (
            config.maturity_level.value
            if hasattr(config.maturity_level, "value")
            else str(config.maturity_level)
        ),
    ...
    try:
        ...
        result = subprocess.run(
            ["git", "worktree", "list"], capture_output=True, text=True, cwd=config.project_root
        )
    ...
    status_data = {
        ...
        "configuration": {
            "maturity_preset": (
                config.maturity_preset.__dict__ if hasattr(config, "maturity_preset") else {}
            ),
            "agent_config": (
                config.agents.model_dump()
                if hasattr(config.agents, "model_dump")
                else config.agents.__dict__
            ),
            "platform_config": (
                config.platforms.model_dump()
                if hasattr(config.platforms, "model_dump")
                else config.platforms.__dict__
            ),
        },
    }
